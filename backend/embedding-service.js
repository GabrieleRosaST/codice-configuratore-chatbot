...
async function getPendingFiles() {
    console.log('[embedding-service] Fetch dei file pending da: MOODLE_API_URL =', getMoodleApiUrl());
    const res = await fetch(getMoodleApiUrl(), {
            headers: {
                'ngrok-skip-browser-warning': 'true'
            }
    });
    console.log('[embedding-service] Status della risposta:', res.status);
    console.log('[embedding-service] Headers risposta:', JSON.stringify(res.headers.raw()));

    // Prova a loggare anche la response in raw text per capire se arriva HTML
    let text;
    try {
        text = await res.clone().text(); // Clona la response per poterla leggere due volte
        console.log('[embedding-service] Body (snippet):', text.slice(0, 200));
    } catch (err) {
        console.error('[embedding-service] Errore leggendo il body della risposta:', err);
    }
/////////////////////////////
    if (!res.ok) {
